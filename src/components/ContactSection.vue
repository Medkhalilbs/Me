<template>
  <section id="contact" class="section">
    ...
    <div class="container">
      <div class="section-badge reveal">{{ meta.badge || '// 09 — CONTACT' }}</div>
      <h2 class="section-title reveal">{{ meta.title || profile?.contact_heading || 'Get In Touch' }}</h2>
      <p class="section-subtitle reveal">{{ meta.subtitle || profile?.contact_subheading || 'Available for remote contracts, IT consulting, and full-stack positions' }}</p>
      ...
      <div class="contact-grid">
        ...
        <div class="contact-info reveal">
          ...
          <p class="info-text">
            {{ profile?.contact_description || 'Have a project requirement or looking to hire a full-stack developer who understands both embedded silicon and cloud orchestration? Send me a line.' }}
          </p>

          <div class="contact-links">
            <a v-if="profile?.email" :href="`mailto:${profile.email}`" target="_blank" class="contact-link navy-hover">
            ...
            <a v-if="profile?.linkedin_url" :href="profile.linkedin_url" target="_blank" rel="noopener" class="contact-link navy-hover">
              <div class="contact-link-icon">
            ...
            <a v-if="profile?.github_url" :href="profile.github_url" target="_blank" rel="noopener" class="contact-link navy-hover">
              <div class="contact-link-icon">
...
const meta = computed(() => store.getSectionMeta('contact'))

const cleanLinkedinLabel = computed(() => {
  if (!profile.value?.linkedin_url) return ''
  return profile.value.linkedin_url.replace(/^https?:\/\/(www\.)?/, '')
})

const cleanGithubLabel = computed(() => {
  if (!profile.value?.github_url) return ''
  return profile.value.github_url.replace(/^https?:\/\/(www\.)?/, '')
})
